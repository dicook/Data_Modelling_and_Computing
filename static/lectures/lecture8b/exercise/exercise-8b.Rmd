...
# clean up the pisa science data
pisa_au_science <- pisa_au %>% 
  filter(science_fun < 5) %>%
  filter(!is.na(science_time)) %>%
...
nrc_joy <- nrc_sent %>% 
  # filter where sentiment is "joy"
  filter(sentiment == ___)

tidy_books %>%
  # look at only the book, "Emma"
  filter(book == ___) %>%
  # join to the nrc_joy data
  inner_join(___) %>%
...
# only select words that are "anger"
nrc_anger <- nrc_sent %>% 
  filter(___)

nrc_surprise <- nrc_sent %>% 
  filter(___)

#see wha thte nrc_sentiment for anger is for emma
tidy_books %>%
  filter() %>%
  inner_join() %>%
...
# do the same thing but for surprise
...
tidy_books_sent <- tidy_books %>%
  # join the sentiment data to the tidy_books
  # then group by book
  # count the sentiments
  # add a new variable that computes the proportion of seniments
...
afl_sentiment <- afl %>% 
  # group by something - the tweeter?
  group_by() %>%
  # unnest the tokens
  unnest_tokens() %>%
