...
---
# Newton: 60% of the time,
![Bimodality](/Users/frazdt/Dropbox/ETC1010/S2_2017_Notes/max_min.png)
...
- `ggplot(mod1, aes(x=mod1$fitted.values,y=mod1$residuals))+geom_point()`

```{r fig.width=6, fig.height=3.5}
#y_hat <- mod1$fitted.values
#res_hat <- mod1$residuals
ggplot(mod1, aes(x=mod1$fitted.values,y=mod1$residuals))+geom_point()
```
---
...
- Maximum: $SOC1<0$, $SOC2<0$, $SOC3>0$
- Minimum: $SOC1>0$, $SOC2>0$, $SOC3<0$
---
## Back to OLS example
...
p<-ggplot(New_Auto, aes(x=New_Auto$horsepower, y=New_Auto$mpg)) + 
  geom_point()
 p+geom_smooth(method='gam',formula=y~x)
ols_fit <- lm(New_Auto$mpg~New_Auto$horsepower,data=New_Auto) 
