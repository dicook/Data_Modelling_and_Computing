...
> STRATUM is processed to create ...
...
## Compute the weighted average of math scores by state. 
...
First, let's calculate the weighted mean for *math* by *state* and *gender*
...
Next, let's `spread` the `gender` the column (remember the _key_ is the variable, and the _value_ is the values used to `spread` into the new variables you create:
...
Now we need to calculate the difference between male and female scores and arrange to find those with the smallest and largest differences - which can be done by arranging

```{r male-female-diff}
pisa_math_wide %>%
  mutate(math_diff = ___ - ___) %>%
  ___(___)
```
...
> The simple regression model shows us  .... 
