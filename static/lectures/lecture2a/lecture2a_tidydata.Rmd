...
```
library(tidyverse)
grad <- read_csv("http://dmac.netlify.com/lectures/data/graduate-programs.csv")
```
...
Read the data in first with this code:

```
library(tidyverse)
melbtemp <- read_fwf("http://dmac.netlify.com/lectures/data/ASN00086282.dly",
  col_positions=fwf_widths(c(11, 4, 2, 4, 
        rep(c(5, 1, 1, 1), 31)), 
      col_names = c("station", "year", "month",
        "variable", paste0("X", 5:128))))
melbtemp <- melbtemp[,c(1:4, seq(5,128,4))] 
melbtemp_wide <- melbtemp %>% 
  gather(day, value, X5:X125) %>%
  mutate(day = sub("X", "", day)) %>%
  mutate(day = as.numeric(day)%/%4) %>%
  mutate(day = as.integer(day)) %>% 
...
You can get the data from  [data/fly_tbl.csv](http://dmac.netlify.com/lectures/data/fly_tbl.csv) 

```{r echo=FALSE, eval=FALSE}
library(tidyverse)
fly_tbl <- read_csv("http://dmac.netlify.com/lectures/data/fly_tbl.csv")
```
...
You can get the data from  [tb.csv](http://dmac.netlify.com/lectures/data/tb.csv) 
