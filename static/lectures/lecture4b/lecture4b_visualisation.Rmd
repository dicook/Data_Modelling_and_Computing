...
- Coordinate systems
- Layering: overlaying multiple data sets, or multiple geometric elements
- Interpreting plots
- Designing good plots
...
- *fixed, equal:* When variables are made on scales that should be comparable, it may be important to reflect this in the axes limits and page space that the plot takes. 
...
```{r}
df <- data.frame(x=runif(100), y=runif(100)*10)
ggplot(df, aes(x=x, y=y)) + geom_point() + coord_fixed()
ggplot(df, aes(x=x, y=y)) + geom_point() + coord_fixed(ratio=0.1)
```
...
- What type of variable should be mapped to what type of plot element
- Colour palettes
- Color blindness
- Pre-attentive
- Proximity
