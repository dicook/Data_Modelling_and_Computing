...
.vhuge.white[
Draw a mental model / concept map of last weeks lecture content on joins.
]
...
.huge[
  ...
  - Common plots for variables (Mapping variables to graphical elements)
    ]
...
- There are several colour blind tested palettes: RColorbrewer has an associated web site [colorbrewer.org](http://colorbrewer2.org) where the palettes are labelled.
- We can explore different forms of colorblindness using the `dichromat` package. 
]
...
.vlarge[
- Puts males and females right beside each other, so the relative heights of the bars can be seen quickly. 
- The answer to the question would be "No, the numbers were similar in youth, but males are more affected with increasing age."
]
...
- If you want to use R / Rstudio on your laptop:
  - Install R + Rstudio (see Ed posting)
  - open R
  - type the following:
  ```r
  # install.packages("usethis")
  library(usethis)
  use_course("bit.ly/etc1010-ex-4b")
  ```
]
