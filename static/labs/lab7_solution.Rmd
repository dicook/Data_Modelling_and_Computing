...
```{r}
engel_csv <- read_csv("engel.csv")
ggplot(engel_csv, aes(x=income,y=foodexp)) + geom_point()
...
# I basically did this in class, so I'm just posting the scripts here. 

model1 <- function(b, data) {
  b[1] + data$income * b[2]+data$income2 * b[3]
}

measure_distance2 <- function(mod, data) {
  diff <- data$foodexp - model1(mod, data)
  ((mean((diff )^2)))
}

bestOLS <- optim(c(0, 0,0), measure_distance2, data = engel_csv)
bestOLS$par
...
fits_lm<- lm(foodexp~income+income2,data=engel_csv)
summary(fits_lm)
...
bestOLS <- optim(c(8, 0,0), measure_distance2, data = engel_csv)
bestOLS$par
...
bestLAD <- optim(c(0, 0,0), measure_distance3, data = engel_csv)
bestLAD$par

```

j. Should we be concered with the starting value issue in `optim()` in the case of LAD regression? *ANS: Yes, optim will only return the global optima in cases where the objective function is globally concave or convex. What the results for different starting values tell us is that there may be regions where the objective function is not globally convex. This is an issue since optim will never leave this local optima.* 

k. Implement one of the methods you discussed in part e. to determine if the estiated regression coefficients change for different starting values. *There are many different correct answers here. Just make sure they implement the ones the suggested in part e. For instance, if they suggested, try different starting values, make sure they tried several different starting values other than the ones I gave. If they said grid search, make sure they implemented it in R. *
