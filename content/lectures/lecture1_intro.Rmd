...
library(vembedr)
embed_vimeo("167367369", width=800, height=600)
```
...
ped_june <- read_csv("http://www.pedestrian.melbourne.vic.gov.au/datadownload/June_2017.csv")
ped_june_m <- ped_june %>% 
...
ggplot(ped_june_m, aes(x=Hour, y=count, 
                       colour=location, 
...
airport <- airport %>%
  select(AIRPORT, LATITUDE, LONGITUDE, AIRPORT_IS_LATEST) %>%
...
ggmap(map) +
  ...
  geom_point(data=filter(N4YRAA_latlon,
             ...
             alpha=0.3, size=1) +
  facet_wrap(~FL_DATE, ncol=7) + xlab("") + ylab("") +
  theme_map()
```
